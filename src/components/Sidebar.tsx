...
export const Sidebar = () => {
  const casinoItems = [
    ...
    { icon: Tv, label: 'Live Shows', count: '4 217' },
    { icon: LayoutGrid, label: 'Slots' },
    { icon: Rocket, label: 'EarnBet Original' },
    { icon: Video, label: 'Live Casino' },
    { icon: Crown, label: 'Exclusive' },
    { icon: Gift, label: 'Lottery' },
  ];

  return (
    <div className="w-64 h-screen bg-[#020a1a] text-[#8a9db7] flex flex-col font-sans relative overflow-hidden border-r border-white/5">
      
      {/* 1. HEADER CONTAINER */}
      <div className="relative w-full pt-8 pb-4 flex flex-col items-center justify-center overflow-hidden">
        ...
        {/* w-fit ensures the container shrinks to text width, items-end aligns 'casino.io' to the right of 'MAX' */}
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-1 custom-scrollbar">
        ...
        <div className="bg-[#0f1d3a]/60 rounded-2xl border border-white/5 shadow-inner backdrop-blur-sm overflow-hidden">
          <button className="w-full flex items-center justify-between px-4 py-4 text-white bg-gradient-to-r from-blue-900/20 to-transparent">
            <span className="font-bold text-[15px]">Casino</span>
            <ChevronUp size={16} className="text-[#4e6285]" />
          </button>
          ...
          <div className="pb-3 px-1">
            {casinoItems.map((item, idx) => (
              <button key={idx} className="w-full flex items-center gap-3 px-4 py-2.5 hover:text-white transition-colors group">
                ...
                {item.count && (
                  <span className="ml-auto text-[10px] bg-[#1e2d4d] text-blue-400 px-2 py-0.5 rounded-md font-bold border border-blue-500/10">
                    {item.count}
                  </span>
                )}
              </button>
